...
目前正式課程為 CEFR A1，共 8 個單元、32 課與 145 個單字出現位置。所有正式 A1 課程都由 `public/data/a1-course-v3.csv` 建立。另有兩個需人工複核的 A2 試行單元，共 8 課與 53 個單字出現位置；它們使用同一份獨立的 `public/data/a2-course-v1.csv`，不會改寫 A1 資料。A2 單元2為「購物與比較」，完成後先停留在試用與人工複核階段。

## 主要功能

- A1／A2 課程目錄、程度切換與循序解鎖。
- 星期、一天時段、月份與家庭成員的相關字詞主題、跨正式答案與語塊別名的搜尋、狀態篩選及課程詳情捷徑。
- 單字回想、三層提示、字義與片語說明。
- 句子重組、閱讀辨識、句型遷移與文章重組。
- KK 音標獨立練習區。
