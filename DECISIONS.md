...
**Status:** Accepted  
**Decision:** `public/data/a1-course-v3.csv` is the only official A1 curriculum. All 32 lessons must be built from it.  
**Reason:** Two hand-maintained sources caused inconsistent lessons and tests.

## ADR-002 — Word Input and Chunk Meaning Are Separate

**Status:** Accepted  
**Decision:** A normal answer row contains exactly one English word. `chunk_*` groups related rows for whole-phrase meaning and teaching notes.  
...
**Reason:** Stabilize the core learning loop before adding unreliable or platform-dependent voice assessment.
...
**Status:** Accepted  
**Decision:** Prefer open-license recorded audio. Use Web Speech API only as a fallback and never claim pending audio is ready. KK symbols are learned separately from A–Z letter names.  
**Reason:** Avoid paid dependencies, copyright risk, and confusion between letter, word, and phonetic-symbol sounds.
...
**Reason:** Automated bulk generation can introduce unlearned vocabulary and mismatched sentence patterns.

## ADR-007 — Documentation Is Working State

**Status:** Accepted  
**Decision:** Every work cycle reads the root knowledge files and updates progress, decisions, plan, tasks, architecture, or changelog when their facts change.  
**Reason:** Stable files are more reliable than long chat history.
...
**Reason:** This supports Chinese/spaced paths and avoids duplicating launch logic.
