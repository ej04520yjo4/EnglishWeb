...
page = sub_once(
    page,
    r'(\{stage === "recall" && \(\s*<section className=")exercise-card(">)',
    r'\1exercise-card recall-card\2',
    "recall card class",
)

recall_stage = page.index('{stage === "recall" && (')
...
# Replace dense word-detail metadata with two essentials plus an optional disclosure.
...
# Guard every other direct Enter handler from keyboard auto-repeat without changing normal Enter behavior.
page = page.replace(
    'if (event.key !== "Enter") return;\n                event.preventDefault();',
    'if (event.key !== "Enter" || event.repeat) return;\n                event.preventDefault();',
)
